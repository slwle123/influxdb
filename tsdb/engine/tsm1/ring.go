...
// entryUnsafe returns the entry for the given key.
// entry is not safe for use by multiple goroutines.
func (r *ring) entryUnsafe(key string) (*entry, bool) {
	return r.getPartition(key).entryUnsafe(key)
}
...
// writeUnsafe writes values to the entry in the ring's partition associated
// with key. If no entry exists for the key then one will be created.
// writeUnsafe is not safe for use by multiple goroutines.
func (r *ring) writeUnsafe(key string, values Values) error {
	return r.getPartition(key).writeUnsafe(key, values)
}
...
// removeUnsafe deletes the entry for the given key.
// removeUnsafe is not safe for use by multiple goroutines.
func (r *ring) removeUnsafe(key string) {
	r.getPartition(key).removeUnsafe(key)
	if r.entryN > 0 {
		atomic.AddInt64(&r.entryN, -1)
	}
}
...
// entryUnsafe returns the partition's entry for the provided key.
// It not safe for use by multiple goroutines.
func (p *partition) entryUnsafe(key string) (*entry, bool) {
	e, ok := p.store[key]
	return e, ok
}
...
// writeUnsafe is equivalent to write.
// writeUnsafe is not safe for use by multiple goroutines.
func (p *partition) writeUnsafe(key string, values Values) error {
	e, ok := p.store[key]
	if ok {
		return e.add(values)
	}
...
// remove deletes the entry associated with the provided key.
// remove is not safe for use by multiple goroutines.
func (p *partition) removeUnsafe(key string) {
	delete(p.store, key)
}
